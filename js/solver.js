...
export class Solver {
    constructor(board) {
        ...
        this.mineProbability = this.boardCurrent.map((row) => row.map((cell) => (cell === FieldType.MINE ? 1 : 0.0)));
    ...
    getPerimeters() {
        ...
        for (let i = 0; i < this.height; i++) {
            for (let j = 0; j < this.width; j++) {
                if ((cellIsANumber(this.boardCurrent[i][j]) || this.boardCurrent[i][j] === FieldType.EMPTY) || visitedMask[i][j] || !this.adjacentToSolid(i, j))
                    continue;
    ...
    getEquation(perimeter) {
        ...
        for (const [x, y] of perimeter) {
            ...
            for (const [dx, dy] of Board.ALL_MOVES) {
                ...
                const ny = y + dy;
                if (this.isValidCell(nx, ny)) {
                    if (this.boardCurrent[nx][ny] === FieldType.MINE_CALCULATED ||
